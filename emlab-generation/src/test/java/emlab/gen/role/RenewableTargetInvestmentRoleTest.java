...
public class RenewableTargetInvestmentRoleTest {
    ...
    public void setUp() throws Exception {
        ...
        windTurbineA1.specifyAndPersist(-3, energyProducer1, powerGridNodeA, wind, false);

        PowerPlant windTurbineA2 = new PowerPlant();
        windTurbineA2.specifyAndPersist(-4, energyProducer2, powerGridNodeA, wind, false);

        PowerPlant windTurbineB1 = new PowerPlant();
        windTurbineB1.specifyAndPersist(-5, energyProducer1, powerGridNodeB, wind, false);

        PowerPlant windTurbineB2 = new PowerPlant();
        windTurbineB2.specifyAndPersist(-10, energyProducer2, powerGridNodeB, wind, false);

        PowerPlant pvA1 = new PowerPlant();
        pvA1.specifyAndPersist(-2, energyProducer1, powerGridNodeA, pv, false);

        PowerPlant pvA2 = new PowerPlant();
        pvA2.specifyAndPersist(-3, energyProducer2, powerGridNodeA, pv, false);

        PowerPlant pvB1 = new PowerPlant();
        pvB1.specifyAndPersist(-5, energyProducer1, powerGridNodeB, pv, false);

        PowerPlant pvB2 = new PowerPlant();
        pvB2.specifyAndPersist(-10, energyProducer2, powerGridNodeB, pv, false);
    ...
    public void testActMethod() {
        ...
        double windCapacity = powerPlantRepository
                .calculateCapacityOfExpectedOperationalPowerPlantsInMarketAndTechnology(marketA, wind, 0);
        assertEquals("Test original wind capacity in market A: ", 400, windCapacity, 0.01);
        assertEquals("Test wind capacity after investment in year 0", 300,
                powerPlantRepository.calculateCapacityOfExpectedOperationalPowerPlantsInMarketAndTechnology(marketA,
                        pv, 0), 0.1);

        genericInvestmentRole.act(rti);

        assertEquals("Test wind capacity after investment in year 3", 400,
                powerPlantRepository.calculateCapacityOfExpectedOperationalPowerPlantsInMarketAndTechnology(marketA,
                        wind, 2), 0.1);
        // Expected value: (Start + ExpectedTimeForWind*Increment: 400 + 3 * 100
        // = 700, but 800 because of plant size.
        assertEquals("Test wind capacity after investment in year 3", 700,
                powerPlantRepository.calculateCapacityOfExpectedOperationalPowerPlantsInMarketAndTechnology(marketA,
                        wind, 3), 0.1);
        ...
        assertEquals("Test wind capacity after investment in year 0", 500,
                powerPlantRepository.calculateCapacityOfExpectedOperationalPowerPlantsInMarketAndTechnology(marketA,
                        pv, 1), 0.1);
    }
    ...
    public void testPowerGenerationTechnologyTargetRepository() {
        ...
        assertEquals("Testing targetRepository", 400, ((StepTrend) pgttWindFromDB.getTrend()).getStart(), 0.01);
        ...
        assertEquals("Testing if expectedInstalledCapacityOfTechnology is adjusted", 500,
                expectedInstalledCapacityOfTechnology, 0.01);
        ...
        assertEquals("Testing if expectedInstalledCapacityOfTechnology is adjusted", 300,
                expectedInstalledCapacityOfTechnology, 0.01);

        double peakPowerPlantCapacityinMarketA = powerPlantRepository
                .calculatePeakCapacityOfOperationalPowerPlantsInMarket(marketA, 0);
