 ...
 */
@RoleComponent
public class ProcessAcceptedPowerPlantDispatchRoleinSR extends AbstractRole<StrategicReserveOperator> implements
    ...
    public void act(StrategicReserveOperator strategicReserveOperator) {
        ...
        for (Segment segment : reps.segmentRepository.findAll()) {
            // logger.warn("Entering Segment Loop" +
            // segment.getLengthInHours());
            SegmentClearingPoint scp = reps.segmentClearingPointRepository
                    .findSegmentClearingPointForMarketSegmentAndTime(getCurrentTick(), segment, market, false);
            ...
            for (PowerPlantDispatchPlan plan : reps.powerPlantDispatchPlanRepository
                ...
                // logger.warn("Entering PPDP LOOP Successfully"
                // +plan.getOldPrice());
                if (plan.getBiddingMarket().getNodeId().intValue() == market.getNodeId().intValue()) {
                    // logger.warn("Bidding Market LOOP entered successfully " +
                    // plan.getBiddingMarket().getName());
                    if (plan.getStatus() >= 2) {
                        // logger.warn("Checking Accepted Bids finding accepted bids "
                        // +plan.getStatus());
                        if (plan.getSRstatus() <= -10) {
                            // logger.warn("Checking SR Status Contracted " +
                            // plan.getSRstatus());
                            double moneyReturned = ((plan.getAcceptedAmount() * scp.getPrice() * segment
                                    .getLengthInHours()) - ((plan.getAcceptedAmount() * plan.getOldPrice() * segment
                                    .getLengthInHours())));
                            // Price mark up
                            // /(plan.getPowerPlant().getOwner().getPriceMarkUp())
                            // logger.warn("Money Earned "
                            // +(plan.getAcceptedAmount()*scp.getPrice()*segment.getLengthInHours()));
                            // logger.warn("Money Kept "+
                            // (plan.getAcceptedAmount()*plan.getOldPrice()*segment.getLengthInHours()));
                            ...
                            // logger.warn("Owner " + plan.getBidder().getName()
                            // + "money After" +plan.getBidder().getCash());

                            reps.nonTransactionalCreateRepository.createCashFlow(plan.getBidder(),
                                    strategicReserveOperator, moneyReturned, CashFlow.STRRESPAYMENT, getCurrentTick(),
                                    plan.getPowerPlant());
