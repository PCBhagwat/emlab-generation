...
public interface PowerPlantDispatchPlanRepository extends GraphRepository<PowerPlantDispatchPlan> {
    ...
    // public Iterable<PowerPlantDispatchPlan>
    ...
    public Iterable<PowerPlantDispatchPlan> findAllPowerPlantDispatchPlanForPowerPlantForSegmentForTime(
    ...
    @Query(value = "result = g.v(plant).in('POWERPLANT_DISPATCHPLAN').as('x').out('SEGMENT_DISPATCHPLAN').filter{it.segmentID==1}.back('x').filter{it.time==time}; if(!result.hasNext()){return null;} else{return result.next();}", type = QueryType.Gremlin)
    public PowerPlantDispatchPlan findOnePowerPlantDispatchPlanForPeakSegmentGivenPowerPlantAndTime(
            @Param("plant") PowerPlant plant, @Param("time") long time);
    ...
    public Iterable<PowerPlantDispatchPlan> findAllPowerPlantDispatchPlansForSegmentForTime(
    ...
    public Iterable<PowerPlantDispatchPlan> findSortedPowerPlantDispatchPlansForSegmentForTime(
            @Param("segment") Segment segment, @Param("time") long time);
    ...
    public Iterable<PowerPlantDispatchPlan> findAllPowerPlantDispatchPlansForPowerPlantForTime(
            @Param("plant") PowerPlant plant, @Param("time") long time);
...
//
// // get incoming bids
// Pipe<Vertex, Vertex> bids = new LabeledEdgePipe("SEGMENT_DISPATCHPLAN",
// LabeledEdgePipe.Step.BOTH_BOTH);
// // filter by time
// Pipe<Vertex, Vertex> timeFilter = new PropertyFilterPipe<Vertex,
// Long>("time", time, FilterPipe.Filter.EQUAL);
// // create pipeline
...
// long time) {
// Pipe<Vertex, Vertex> bids = new LabeledEdgePipe("POWERPLANT_DISPATCHPLAN",
// LabeledEdgePipe.Step.BOTH_BOTH);
// // filter by time
// Pipe<Vertex, Vertex> timeFilter = new PropertyFilterPipe<Vertex,
// Long>("time", time, FilterPipe.Filter.EQUAL);
...
// Pipe<Vertex, Vertex> bids = new LabeledEdgePipe("BIDDER",
// LabeledEdgePipe.Step.BOTH_BOTH);
// // filter by time
// Pipe<Vertex, Vertex> timeFilter = new PropertyFilterPipe<Vertex,
// Long>("time", time, FilterPipe.Filter.EQUAL);
